...
export function useDeltaTimestamps(): [number, number, number, number] {
  ...
  const t24h = utcCurrentTime.subtract(1, 'day').startOf('minute').unix()
  const t48h = utcCurrentTime.subtract(2, 'day').startOf('minute').unix()
  const t7d = utcCurrentTime.subtract(1, 'week').startOf('minute').unix()
  const t14d = utcCurrentTime.subtract(2, 'week').startOf('minute').unix()
