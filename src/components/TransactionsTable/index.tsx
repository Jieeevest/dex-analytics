...
import { DarkOutlineCard } from 'components/Card'
  ...
  return (
    <>
      ...
      <Wrapper>
        <AutoColumn gap="16px">
          <ResponsiveGrid>
            <ClickableText color={theme.text2} onClick={() => handleSort(SORT_FIELD.amountUSD)} end={0}>
              Actions {arrow(SORT_FIELD.amountUSD)}
            </ClickableText>
          ...
          <PageButtons>
            ...
            >
              <ImageWrap size="20px" src={Assets.ChevronLeft} />
            </Arrow>
            ...
            >
              <ImageWrap size="20px" src={Assets.ChevronRight} />
            </Arrow>
          </PageButtons>
        </AutoColumn>
      </Wrapper>
...
const Wrapper = styled(DarkOutlineCard)`
  width: 100%;
`
...
const SortText = styled.button<{ active: boolean; marginRight: string }>`
  cursor: pointer;
  font-weight: ${({ active }) => (active ? 600 : 500)};
  padding: 8px 16px;
  margin-right: ${({ marginRight }) => marginRight ?? '0.5rem !important'};
  border: none;
