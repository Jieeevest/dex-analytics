...
import { bscBlockClient, litedexClient, biswapClient, pancakeClient } from 'apollo/client'
import { DexInfo, SupportedDex } from 'constants/networks'
...
// returns a function that allows adding a popup
export function useActiveNetworkVersion(): [DexInfo, (activeNetworkVersion: DexInfo) => void] {
  const dispatch = useDispatch()
  const activeNetwork = useSelector((state: AppState) => state.application.activeNetworkVersion)
  const update = useCallback(
    (activeNetworkVersion: DexInfo) => {
...
export function useDataClient(): ApolloClient<NormalizedCacheObject> {
  ...
  switch (activeNetwork.id) {
    case SupportedDex.LITEDEX:
      return litedexClient
    case SupportedDex.BISWAP:
      return biswapClient
    case SupportedDex.PANCAKESWAP:
