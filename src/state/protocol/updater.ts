...
export default function Updater(): null {
  ...
  const [protocolData, updateProtocolData] = useProtocolData() //undefined

  const { data: fetchedProtocolData, error, loading } = useFetchProtocolData() //undefined

  const [chartData, updateChartData] = useProtocolChartData() //gonna check
