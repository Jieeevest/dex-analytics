...
export function useTVLOffset() {
  ...
  const tvlOffset = useMemo(() => {
    ...
    return Object.keys(data).reduce((accum: number, poolAddress) => {
      ...
      return accum + poolData.liquidityUSD
    }, 0)
  }, [data])
  return tvlOffset
}
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      const data = await POOL_HIDE.reduce(async (accumP: Promise<{ [key: number]: ChartDayData }>, address) => {
        ...
        dispatch(updatePoolChartData({ poolAddress: address, chartData: data, networkId: SupportedDex.LITEDEX }))
        data.map((poolDayData: PoolChartEntry) => {
          const { date, liquidityUSD, volumeUSD } = poolDayData
          ...
          accum[roundedDate].liquidityUSD = accum[roundedDate].liquidityUSD + liquidityUSD
