...
export function usePoolDatas(poolAddresses: string[]): {
  ...
  const parsed = data?.pairs
    ...
    : {}
  const parsed24h = data24h?.pairs
    ? data24h.pairs.reduce((accum: { [address: string]: PoolFields }, poolData) => {
        accum[poolData.id] = poolData
        return accum
      }, {})
    : {}
  const parsed48h = data48h?.pairs
    ? data48h.pairs.reduce((accum: { [address: string]: PoolFields }, poolData) => {
        accum[poolData.id] = poolData
        return accum
      }, {})
    : {}
  const parsed7d = data7d?.pairs
    ? data7d.pairs.reduce((accum: { [address: string]: PoolFields }, poolData) => {
        accum[poolData.id] = poolData
        return accum
      }, {})
    : {}
  const parsed14d = data14d?.pairs
    ? data14d.pairs.reduce((accum: { [address: string]: PoolFields }, poolData) => {
        accum[poolData.id] = poolData
        return accum
      }, {})
    : {}
